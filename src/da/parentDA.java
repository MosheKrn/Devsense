...
public class parentDA  implements parentdal {
	...
	try{
	String query="INSERT INTO public.parent (parent_id, parent_name) VALUES (?,?)";
	...
	try
	{
	String query = "UPDATE  public.parent SET parent_name= ? where parent_id=?";
	
        PreparedStatement preparedStatement = conn.prepareStatement(query);
	
	
	preparedStatement.setString(1,parent.getParentName());
	preparedStatement.setInt(2,parent.getParentID());
